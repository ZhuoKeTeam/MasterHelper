 ...
 */
package com.team.zhuoke.masterhelper.utils.backfragmentutils;
...
public class BackFragmentHelper {
    ...
    private static boolean fireOnBackPressedEvent(List<?> gFragmentList) {
        ...
        Collections.sort(backFragmentList, (lhs, rhs) -> rhs.getBackPriority() - lhs.getBackPriority());

        // send them onBackPressed event
        boolean handled = false;
        for (BackFragment f : backFragmentList) {
            if (f instanceof Fragment && ((Fragment) f).isVisible()) {
